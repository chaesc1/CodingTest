...
def solution(s):
    answer = [] #끊을 수 있는 개수를 담을 배열
    for i in range(1,len(s)+1): #i자 씩 끊어가
        compressed = []
        count = 1 #첫단어를 카운트한채로
        tmp = s[:i]#첫 번째 단어 미리 자르기

        for j in range(i,len(s)+i,i): #i부터 끝까지 i단위로 나누어 탐색
            if tmp==s[i:i+j]: #앞문자와 같다면?
                count+=1 #카운트 늘려줘
            else: #다르면?
                if count != 1:#
                    compressed = compressed + str(count)+tmp
                else:
                    compressed = compressed + tmp
                
                tmp = s[j+j+i] #다시 뒤부터 잘라
                cnt = 1 #1로 초기화
